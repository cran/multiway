parafac_3wayna <-
  function(data,nfac,naid=which(is.na(data)),const=rep(0L,3),
    ...
    pfac <- list(A=Anew,B=Bnew,C=Cnew,SSE=ssenew,Rsq=Rsq,GCV=GCV,edf=edf,iter=iter,
                 cflag=cflag,const=const,control=control,fixed=fixed,struc=struc)
    return(pfac)
