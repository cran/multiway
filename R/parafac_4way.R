parafac_4way <-
  function(data, nfac, xcx = sumsq(data), const = rep("uncons", 4),
    ...
    Xa <- t(matrix(data, nrow = xdims[1], ncol = xdims[2] * xdims[3] * xdims[4]))
    if(is.null(Bfixed)) Xb <- t(matrix(aperm(data, perm = c(2,1,3,4)), nrow = xdims[2], ncol = xdims[1] * xdims[3] * xdims[4]))
    if(is.null(Cfixed)) Xc <- t(matrix(aperm(data, perm = c(3,1,2,4)), nrow = xdims[3], ncol = xdims[1] * xdims[2] * xdims[4]))
    if(is.null(Dfixed)) Xd <- t(matrix(aperm(data, perm = c(4,1,2,3)), nrow = xdims[4], ncol = xdims[1] * xdims[2] * xdims[3]))
    rm(data)
    ...
    while((vtol > ctol) && (iter < maxit)) {
      ...
      if(is.null(Afixed) && is.na(cflag)){
        ...
        Amat <- t(cmls(X = DCkrB, Y = Xa, const = const[1], struc = Astruc,
      ...
      if(is.null(Bfixed) && is.na(cflag)){
        ...
        Bmat <- t(cmls(X = DCkrA, Y = Xb, const = const[2], struc = Bstruc,
      ...
      if(is.null(Cfixed) && is.na(cflag)){
        ...
        Cmat <- t(cmls(X = DBkrA, Y = Xc, const = const[3], struc = Cstruc,
      ...
      if(is.null(Dfixed) && is.na(cflag)){
        ...
        Dmat <- t(cmls(X = CBkrA, Y = Xd, const = const[4], struc = Dstruc,
      ...
      for(u in 1:nfac) DCkrB[,u] <- kronecker(Dmat[,u], kronecker(Cmat[,u], Bmat[,u]))
      #ssenew <- sum((Xa - tcrossprod(Amat, DCkrB))^2)
      ssenew <- sum((Xa - tcrossprod(DCkrB, Amat))^2)
